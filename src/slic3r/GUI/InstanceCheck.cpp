...
bool instance_check(int argc, char** argv, bool app_config_single_instance)
{
...
#ifdef _WIN32
    // TODO switch when windows compiler will allow it
	//hashed_path = std::hash<std::string>{}(std::filesystem::system_complete(std::filesystem::path(argv[0])).string());
	...
	if (! appimage_env_valid)
#endif // __linux__
		hashed_path = std::hash<std::string>{}(std::filesystem::canonical(std::filesystem::system_complete(argv[0]), ec).string());
	if (ec.value() > 0) { // canonical was not able to find the executable (can happen with appimage on some systems. Does it fail on Fuse file systems?)
		...
		hashed_path = std::hash<std::string>{}(std::filesystem::canonical(std::filesystem::system_complete(argv[0]).parent_path(), ec).string() + "/" + std::filesystem::system_complete(argv[0]).filename().string());
		if (ec.value() > 0) {
			// Still not valid, process without canonical
			hashed_path = std::hash<std::string>{}(std::filesystem::system_complete(argv[0]).string());
		}
	}
#endif // _WIN32

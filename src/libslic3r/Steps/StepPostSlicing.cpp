    ...
    for (size_t object_idx = 0; object_idx < print.objects().size(); ++object_idx) {
        ...
        for (size_t layer_idx = 0; layer_idx < object->layer_count(); ++layer_idx) {
            ...
            for (size_t island_idx = 0; island_idx < layer->islands().size(); ++island_idx) {
                const LayerSliceIsland *island = layer->islands()[island_idx].get();
                if (island == nullptr) {
                    ok = false;
                    out_error += layer_prefix.str() + "island is null";
                    continue;
                }
            ...
            for (size_t region_idx = 0; region_idx < layer->region_count(); ++region_idx) {
                ...
                if (region == nullptr) {
                    ok = false;
                    out_error += layer_prefix.str() + "layer region is null";
                    continue;
                }
...
run_ctx_post_slicing make_post_slicing_run_context(Print &print, size_t object_idx)
{
    ...
    context_step.object = reinterpret_cast<object_handle *>(print.get_object(object_idx));
...
void run_step(Orchestrator &orchestrator, Print &print)
{
    ...
    for (Plugin *plugin : plugins) {
        ...
        parallel_for(size_t(0), run_count, [&print, plugin, &run_context](const size_t idx) {
            ...
            run_ctx_post_slicing context_step = make_post_slicing_run_context(print, idx);

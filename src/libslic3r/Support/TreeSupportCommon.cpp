...
//    const std::vector<unsigned int>  printing_extruders = print_object.object_extruders();
    ...
    if (config.support_material_contact_distance_type.value == zdNone) {
        ...
        print_object.layers().front().region_count() > 0) {
...
//    this->support_roof_angles       = 
    ...
    this->support_line_spacing      = scale_i(config.support_material_spacing.value);
//    this->support_bottom_offset     = 
//    this->support_wall_count        = config.support_material_with_sheath ? 1 : 0;
    this->support_wall_count        = 1;
    this->support_roof_line_distance = scale_i(config.support_material_interface_spacing.value) + this->support_roof_line_width;
//    this->minimum_support_area      = 
//    this->minimum_bottom_area       = 
//    this->support_offset            = 
    this->support_tree_branch_distance = scale_i(config.support_tree_branch_distance.value);
    ...
    this->support_tree_branch_diameter = scale_i(config.support_tree_branch_diameter.value);
...
//    this->support_tree_tip_diameter = this->support_line_width;
    this->support_tree_tip_diameter = std::clamp(scale_i(config.support_tree_tip_diameter.value), coord_t(0), this->support_tree_branch_diameter);
}
      ...
      min_feature_size(mesh_group_settings.min_feature_size)
{
    ...
    if (TreeSupportSettings::soluble) {
        ...
        xy_min_distance = std::max(xy_min_distance, scale_i(0.1));
